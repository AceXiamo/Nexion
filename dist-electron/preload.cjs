...
electron.contextBridge.exposeInMainWorld("ipcRenderer", {
  ...
  // SSH Tab 相关方法
  ssh: {
    createSession: (config) => electron.ipcRenderer.invoke("ssh-create-session", config),
    closeSession: (sessionId) => electron.ipcRenderer.invoke("ssh-close-session", sessionId),
    switchSession: (sessionId) => electron.ipcRenderer.invoke("ssh-switch-session", sessionId),
    sendCommand: (sessionId, command) => electron.ipcRenderer.invoke("ssh-send-command", sessionId, command),
    resizeSession: (sessionId, cols, rows) => electron.ipcRenderer.invoke("ssh-resize-session", sessionId, cols, rows),
    getAllSessions: () => electron.ipcRenderer.invoke("ssh-get-all-sessions"),
    getActiveSession: () => electron.ipcRenderer.invoke("ssh-get-active-session"),
    testConnection: (config) => electron.ipcRenderer.invoke("ssh-test-connection", config)
  }
});
